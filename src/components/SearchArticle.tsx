...
const SearchArticle = () => {
  ...
  const searchArticle = async () => {
    ...
    setError(null);
    try {
      const article = await googleSheetsService.getArticleByNumber(selectedLawIndex, articleNumber.trim());
      if (!article) {
      ...
      <AnimatePresence mode="wait">
              ...
              <div className="space-y-4">
                <div className="space-y-2 py-[20px] px-0">
                  ...
                  <Select value={selectedLawIndex !== null ? selectedLawIndex.toString() : ""} onValueChange={value => setSelectedLawIndex(Number(value))}>
                    ...
                    <SelectContent>
                      {laws.map((law, index) => <SelectItem key={index} value={index.toString()}>
                          {law.title}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-2 py-[10px] px-0 my-[25px]">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button onClick={searchArticle} disabled={isLoading} className="gap-2">
                      {isLoading ? <div className="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full" /> : <Search className="h-4 w-4" />}
                      Buscar
                    </Button>
                  </div>
                </div>
        ...
        opacity: 0
            ...
            <div className="flex items-center mb-4 px-4">
              <Button variant="ghost" onClick={() => setSearchedArticle(null)} className="gap-2">
                ← Voltar à pesquisa
              </Button>
            </div>
            
            <ArticleView articleNumber={searchedArticle.number} content={searchedArticle.content} lawTitle={laws[selectedLawIndex!]?.title || ""} />
          </motion.div>}
      </AnimatePresence>
    </div>;
};
